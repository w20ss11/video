...
import com.example.video.camera.FrameCallback;
import com.example.video.camera.Renderer;
import com.example.video.camera.TextureController;
import com.example.video.filter.ZipPkmAnimationFilter;
import com.example.video.utils.DoImgUtils;
import com.example.video.utils.PermissionUtils;
...
public class PreviewActivity extends AppCompatActivity implements FrameCallback {
...
//        setContentView(R.layout.activity_preview);
    ...
    public void onClick(View view){
        switch (view.getId()){
            case R.id.mShutter:
                DoImgUtils doImgUtils = new DoImgUtils();
                mController.takePhoto();
                String text = doImgUtils.test();
                Toast.makeText(PreviewActivity.this, "res is " + text, Toast.LENGTH_SHORT).show();
                break;
        }
    }
    ...
    public void saveBitmap(Bitmap b){
        ...
        if(!folder.exists()&&!folder.mkdirs()){
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(PreviewActivity.this, "无法保存照片", Toast.LENGTH_SHORT).show();
                }
            });
            return;
        }
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(PreviewActivity.this, "保存成功->"+jpegName, Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private Runnable initViewRunnable = new Runnable() {
        ...
        public void run() {
            ...
            mController = new TextureController(PreviewActivity.this);
...
//            mController.addFilter(filter);
            ...
            mController.setFrameCallback(720, 1280, PreviewActivity.this);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        PermissionUtils.onRequestPermissionsResult(requestCode == 10, grantResults, initViewRunnable,
            new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(PreviewActivity.this, "没有获得必要的权限", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        );
